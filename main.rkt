...
(define-struct birds (x y v-y))
(define-struct pipe (gap-y x))
(define-struct variables (gravity momentum scroll-speed))
(define-struct states (bird pipe variables score))
(define (get-initial-state)
  (define flappy (birds bird-x bird-initial-y 0))
  (define p (pipe (+ added-number (random random-threshold)) scene-width))
  (define pipes (list p))
  (define var (variables initial-gravity initial-momentum initial-scroll-speed))
  (define initial (states flappy pipes var 0))
  initial)
...
(define (next-state-bird bird gravity)
  (struct-copy birds bird [y (+ (get-bird-y bird) (get-bird-v-y bird))]
...
(define (next-state-bird-onspace bird momentum)
  (match-let ([(birds _ _ v-y) bird] [momentum momentum])
    (struct-copy birds bird [v-y (- momentum)])))
...
(define (change current-state pressed-key)
  (if (key=? pressed-key " ")
      (struct-copy states current-state
...
(define (add-more-pipes pipes)
  (define new-pipe (pipe (+ added-number (random random-threshold))
                         (+ pipe-width pipe-gap (pipe-x (last pipes)))))
  (if (> no-pipes (length pipes))
      (reverse (cons new-pipe (reverse pipes)))
      pipes))
...
(define (check-ground-collision bird)
  (if (>= (+ (birds-y bird) bird-height) ground-y)
      #t
      #f))
...
(define (invalid-state? state)
  (or (check-ground-collision (states-bird state))
      (check-pipe-collisions (states-bird state) (states-pipe state))))
...
(define (check-pipe-collisions bird pipes)
  (define check-upper (λ (p)
                        (check-collision-rectangles
                         (make-posn (birds-x bird) (birds-y bird))
                         (make-posn (+ (birds-x bird) bird-width)
                                    (+ (birds-y bird) bird-height))
                         (make-posn (pipe-x p) 0)
                         (make-posn (+ (pipe-x p) pipe-width)
                                    (pipe-gap-y p)))))
  (define check-lower (λ (p)
                        (check-collision-rectangles
                         (make-posn (birds-x bird) (birds-y bird))
                         (make-posn (+ (birds-x bird) bird-width)
                                    (+ (birds-y bird) bird-height))
                         (make-posn (pipe-x p)
                                    (+ (pipe-gap-y p) pipe-self-gap))
                         (make-posn (+ (pipe-x p) pipe-width)
                                    scene-height))))
  (if (null? pipes)
      #f
      (if (or (check-upper (car pipes)) (check-lower (car pipes)))
          #t
...
(define (next-state state)
  (struct-copy states state
               [bird (next-state-bird
                      (get-bird state) (variables-gravity (states-variables state)))]
               [pipe (next-state-pipes
                      (get-pipes state) (variables-scroll-speed (states-variables state)))]
  ...
  (place-image bird-image
               ...
               (place-image ground-image
                            ...
                            (place-image (score-to-image (get-score state))
                                         text-x text-y
                                         (place-pipes (states-pipe state) initial-scene)))))

; Folosind `place-image/place-images` va poziționa pipe-urile pe scenă.
(define (place-pipes pipes scene)
  (define images (list (rectangle pipe-width pipe-height "solid" "green")
                       (rectangle pipe-width pipe-height "solid" "green")))
  (foldl (λ (p s) (place-images
                   images
                   (list (make-posn (+ (get-pipe-x p) (quotient pipe-width 2))
                                    (- (pipe-gap-y p) (quotient pipe-height 2)))
                         (make-posn (+ (get-pipe-x p) (quotient pipe-width 2))
                                    (+ (pipe-gap-y p) pipe-self-gap
